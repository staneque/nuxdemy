    ...
    <div class="bg-slate-200 p-8 rounded-xl w-full max-w-2xl">
      ...
      <form v-else @submit.prevent="handleSubmit">
        ...
        <div class="mt-8 text-base width bg-white py-6 px-8 rounded shadow-md">
          ...
          <div class="text-red-600 py-4" v-if="paymentIntentError">
            {{ paymentIntentError }}
          </div>
        </div>
...
const setupStripe = () => {
  ...
  if (!card.value && elements.value) {
    ...
    card.value.on('change', function (event) {
      if (event.error) {
        paymentIntentError.value = event.error.message
      } else {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    // if (response.error) {
    //   paymentIntentError.value = response.error.message + ' lol'
